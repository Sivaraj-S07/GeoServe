...
function StatCard({ label, value, sub, icon, accent }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontFamily: "'Manrope','Manrope',sans-serif", fontWeight: 900, fontSize: 26, color: a.val, letterSpacing: "-1px", marginBottom: 4 }}>
        {value}
      </div>
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,.45)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: "var(--surface)", borderRadius: 18, padding: "28px 28px", width: "100%", maxWidth: 420, boxShadow: "0 20px 60px rgba(15,23,42,.2)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <h3 style={{ fontFamily: "'Manrope',sans-serif", fontWeight: 800, fontSize: 18, margin: 0 }}>{t("adminPayments.recordWithdrawal")}</h3>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          <button
            ...
            style={{ flex: 1, padding: "11px 0", borderRadius: 9, border: "none", background: busy ? "#a5b4fc" : "linear-gradient(135deg,#2563eb,#6366f1)", color: "#fff", fontSize: 13, fontWeight: 700, cursor: busy ? "not-allowed" : "pointer", fontFamily: "inherit" }}
          >
  ...
  return (
    <div className="anim-fade">
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
        <div>
          <h2 style={{ fontWeight: 800, fontSize: 22, letterSpacing: -.5, margin: "0 0 4px" }}>{t("adminPayments.commissionWallet")}</h2>
      ...
      {summary?.topCategories?.length > 0 && (
        <div className="card" style={{ padding: "20px 24px", marginBottom: 24 }}>
          <h3 style={{ fontWeight: 700, fontSize: 14, margin: "0 0 16px", fontFamily: "'Manrope',sans-serif" }}>{t("adminPayments.topEarningCategories")}</h3>
              ...
              return (
                <div key={c.category} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  <div style={{ flex: 2, height: 6, background: "var(--surface-raised)", borderRadius: 99, overflow: "hidden" }}>
                    <div style={{ height: "100%", width: `${pct}%`, background: "linear-gradient(90deg,#2563eb,#818cf8)", borderRadius: 99 }} />
                  </div>
                  <span style={{ fontSize: 12, fontWeight: 700, color: "#2563eb", width: 60, textAlign: "right" }}>{fmt(c.total)}</span>
      ...
      <div className="card" style={{ padding: "20px 24px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 10 }}>
          <h3 style={{ fontWeight: 700, fontSize: 15, margin: 0, fontFamily: "'Manrope',sans-serif" }}>
            Transaction Ledger
          ...
          >
            <option value="">{t("adminPayments.allTypes")}</option>
            <option value="credited">{t("adminPayments.credited")}</option>
            <option value="pending_retry">{t("adminPayments.pendingRetry")}</option>
            <option value="withdrawal">{t("adminPayments.withdrawals")}</option>
          </select>
        </div>
        ...
        {txns.length === 0 ? (
          <div style={{ textAlign: "center", padding: "40px 0", color: "var(--muted)" }}>
            ...
            <p style={{ marginTop: 10, fontWeight: 500 }}>{t("adminPayments.noTransactions")}</p>
          </div>
        ) : (
          <>
                  ...
                  <div key={t.id} style={{
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 12, color: "var(--muted)", display: "flex", gap: 12, flexWrap: "wrap" }}>
                        ...
                        {!isWithdrawal && t.serviceCost > 0 && <span>Service: {fmt(t.serviceCost)}</span>}
                        {!isWithdrawal && t.distanceCost > 0 && <span>🚗 Distance: {fmt(t.distanceCost)}</span>}
