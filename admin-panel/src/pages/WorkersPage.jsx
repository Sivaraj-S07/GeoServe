...
function WorkerCard({ worker, category, categoryObj, onApprove, onDelete, approving, deleting }) {
  const { i18n } = useTranslation();
  // Display name — falls back to the legacy name for worker profiles.
  ...
  return (
    ...
    >
      {/* Top gradient bar */}
      <div style={{ height:4,background:worker.approved?"linear-gradient(90deg,var(--blue),#7c3aed)":"linear-gradient(90deg,#f59e0b,#d97706)" }} />
      ...
      <div style={{ padding:"18px 20px",flex:1 }}>
        ...
        <div style={{ display:"flex",alignItems:"flex-start",gap:14,marginBottom:14 }}>
          ...
          <div style={{ flex:1,minWidth:0 }}>
            <div style={{ fontWeight:800,fontSize:15,marginBottom:4,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{displayName}</div>
            <div style={{ display:"flex",gap:6,flexWrap:"wrap" }}>
              {category && <span style={{ background:"var(--blue-bg)",border:"1px solid #bfdbfe",color:"#1d4ed8",fontSize:11,fontWeight:700,padding:"2px 8px",borderRadius:20,display:"inline-flex",alignItems:"center",gap:4 }}><Icon name={getCategoryIcon(categoryObj?.icon||category)} size={10} color="#1d4ed8" strokeWidth={2}/>{category}</span>}
              <span style={{ background:online?"var(--green-bg)":"#f8fafc",border:`1px solid ${online?"var(--primary-border)":"#e2e8f0"}`,color:online?"var(--primary)":"#94a3b8",fontSize:11,fontWeight:700,padding:"2px 8px",borderRadius:20 }}>
        ...
        <div style={{ display:"flex",flexDirection:"column",gap:6,marginBottom:14 }}>
          {[
            ...
            { icon:"🪪", val:worker.aadhaarNumber ? worker.aadhaarNumber.replace(/(\d{4})(\d{4})(\d{4})/, "$1 $2 $3") + " (Aadhaar)" : null },
        ...
        <div style={{ display:"flex",gap:10,marginBottom:14 }}>
          ...
          {worker.jobsCompleted>0 && (
            <div style={{ flex:1,textAlign:"center",padding:"8px",background:"var(--green-bg)",borderRadius:10,border:"1px solid var(--primary-border)" }}>
              <div style={{ fontSize:16,fontWeight:900,color:"var(--primary)" }}>{worker.jobsCompleted}</div>
              <div style={{ fontSize:10,fontWeight:700,color:"var(--primary-dark)",textTransform:"uppercase" }}>Jobs</div>
            </div>
          )}
          {worker.yearsOfExp>0 && (
            <div style={{ flex:1,textAlign:"center",padding:"8px",background:"var(--blue-bg)",borderRadius:10,border:"1px solid #bfdbfe" }}>
              <div style={{ fontSize:16,fontWeight:900,color:"var(--blue)" }}>{worker.yearsOfExp}y</div>
      ...
      <div style={{ padding:"12px 20px",borderTop:`1px solid ${worker.approved?"#f3f4f6":"#fde68a"}`,background:worker.approved?"#fafafa":"#fffbeb",display:"flex",gap:8 }}>
        {!worker.approved ? (
          <button disabled={approving} onClick={()=>onApprove(worker.id)}
            style={{ flex:1,padding:"9px",borderRadius:10,border:"none",background:approving?"var(--green-soft)":"linear-gradient(135deg,#2563eb,#3b82f6)",color:"white",fontWeight:800,fontSize:13,cursor:approving?"not-allowed":"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:7,boxShadow:approving?"none":"0 4px 14px rgba(37,99,235,.35)",transition:"all .15s" }}
          ...
          <div style={{ flex:1,display:"flex",alignItems:"center",gap:7,padding:"8px 12px",background:"var(--green-bg)",borderRadius:10,border:"1px solid var(--primary-border)" }}>
            <span style={{ fontSize:14 }}>✅</span>
            <span style={{ fontSize:12,fontWeight:700,color:"var(--primary)" }}>Approved</span>
          </div>
        )}
        <button disabled={deleting}
          onClick={()=>{ if(window.confirm(`Delete ${displayName}? This cannot be undone.`)) onApprove(worker.id,"delete"); }}
          style={{ padding:"9px 12px",borderRadius:10,border:"1.5px solid var(--red-border)",background:"var(--red-bg)",color:"var(--red)",fontSize:13,cursor:deleting?"not-allowed":"pointer",fontFamily:"inherit",transition:"all .15s",fontWeight:700 }}
          title="Delete worker"
          aria-label={`Delete worker ${displayName}`}
        >🗑</button>
      </div>
    </div>
  );
...
export default function WorkersPage({ onToast }) {
  ...
  const load = useCallback(async (silent=false) => {
    ...
    } catch { if (!silent) onToast(t("adminWorkers.noWorkers"),"error"); }
  ...
  const handleApprove = async (id) => {
    setBusy(p=>({...p,[id]:true}));
    try { const updated=await api.approveWorker(id); setWorkers(p=>p.map(w=>w.id===id?updated:w)); onToast(t("adminWorkers.deleteSuccess")); }
    catch { onToast("Failed to approve","error"); }
    finally { setBusy(p=>({...p,[id]:false})); }
  };
  const handleDelete = async (id) => {
    setBusy(p=>({...p,[`del_${id}`]:true}));
    try { await api.deleteWorker(id); setWorkers(p=>p.filter(w=>w.id!==id)); onToast(t("adminWorkers.deleteSuccess")); }
  ...
  const filtered = workers.filter(w=>{
    ...
    const mQ=!q||w.name?.toLowerCase().includes(q)||w.nameEn?.toLowerCase().includes(q)||w.email?.toLowerCase().includes(q)||w.specialization?.toLowerCase().includes(q);
  ...
  return (
    <div className="anim-fade" style={{ padding:"28px 32px" }}>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:12,marginBottom:20 }}>
        {[
          { label:"Total",    val:workers.length,   grad:"linear-gradient(135deg,var(--primary),#7c3aed)",bg:"var(--primary-bg)",border:"var(--primary-border)" },
          { label:"Approved", val:workers.filter(w=>w.approved).length, grad:"linear-gradient(135deg,#2563eb,#3b82f6)",bg:"var(--green-bg)",border:"var(--primary-border)" },
          { label:"Pending",  val:pendingCount,     grad:"linear-gradient(135deg,#d97706,#f59e0b)",bg:"#fffbeb",border:"#fde68a" },
          { label:"Online",   val:onlineCount,      grad:"linear-gradient(135deg,var(--primary),#34d399)",bg:"var(--green-bg)",border:"var(--primary-border)" },
        ].map(s=>(
          <div key={s.label} style={{ background:"white",borderRadius:14,border:`1.5px solid ${s.border}`,padding:"14px 16px",position:"relative",overflow:"hidden",cursor:"default" }}>
            ...
            <div style={{ paddingTop:4 }}>
              <div style={{ fontSize:24,fontWeight:900,letterSpacing:-1,color:"#0c1a3d" }}>{s.val}</div>
      ...
      <div style={{ background:"white",border:"1.5px solid #e2e8f0",borderRadius:14,padding:"12px 16px",marginBottom:20,display:"flex",gap:10,flexWrap:"wrap",alignItems:"center",boxShadow:"0 2px 8px rgba(0,0,0,.04)" }}>
        <div style={{ position:"relative",flex:1,minWidth:200 }}>
          ...
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder={t("adminWorkers.searchPlaceholder")}
      ...
      {loading ? (
        <div style={{ display:"flex",alignItems:"center",justifyContent:"center",padding:64,gap:14 }}>
          <div style={{ width:32,height:32,borderRadius:"50%",border:"3px solid var(--primary-bg)",borderTopColor:"var(--primary)",animation:"spin .7s linear infinite" }} />
        ...
        <div style={{ textAlign:"center",padding:"64px 24px",background:"white",borderRadius:18,border:"1.5px solid #e2e8f0" }}>
          ...
          <div style={{ fontWeight:700,fontSize:16,color:"#0c1a3d",marginBottom:6 }}>No workers found</div>
          <div style={{ fontSize:13,color:"#94a3b8" }}>Try adjusting your filters</div>
        </div>
      ) : viewMode==="grid" ? (
        <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:18 }}>
          {filtered.map(w=>(
            <WorkerCard key={w.id} worker={w} category={getCat(w.categoryId)?.name} categoryObj={getCat(w.categoryId)}
          ...
          <table className="data-table">
            ...
            <tbody>
              {filtered.map(w=>{
                ...
                const wDisplayName = getLocalizedName(w, i18n.language) || w.name || "";
                const fb=`https://ui-avatars.com/api/?name=${encodeURIComponent(wDisplayName)}&background=2563eb&color=fff&size=80`;
                return (
                  <tr key={w.id} onMouseEnter={e=>{[...e.currentTarget.querySelectorAll("td")].forEach(td=>td.style.background="#f8faff");}} onMouseLeave={e=>{[...e.currentTarget.querySelectorAll("td")].forEach(td=>td.style.background="");}}>
                    <td>
                      <div style={{ display:"flex",alignItems:"center",gap:10 }}>
                        ...
                        <div>
                          <div style={{ fontWeight:700,fontSize:13 }}>{wDisplayName}</div>
                    ...
                    <td>
                      ...
                      <div style={{ fontSize:11,color:"#94a3b8" }}>{w.phone || "Not provided"}</div>
                    </td>
                    ...
                    <td>
                      <div style={{ display:"flex",gap:6 }}>
                        ...
                        <button disabled={busy[`del_${w.id}`]} onClick={()=>{ if(window.confirm(`Delete ${wDisplayName}?`))handleDelete(w.id); }} className="btn btn-red btn-sm" style={{ fontSize:12 }}>{busy[`del_${w.id}`]?"…":"🗑"}</button>
                      </div>
                    </td>
                  </tr>
                );
