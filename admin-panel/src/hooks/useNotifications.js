...
function buildMessage(payload) {
  ...
  if (!b) return "New notification";
  if (payload.type === "new_booking") return `New booking from ${b.userName || "a user"} — ${b.category || "Service"}`;
  if (payload.type === "booking_update") return `Booking #GS${b.id} is now ${(payload.status || b.status).replace("_", " ")}`;
