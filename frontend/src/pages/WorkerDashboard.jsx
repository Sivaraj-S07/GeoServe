          ...
          {tab === "payout" && (
            <>
              ...
              <div className="card" style={{ padding:28,maxWidth:560 }}>
                <h3 style={{ fontWeight:800,fontSize:16,marginBottom:20 }}>{t("workerDashboard.bankAccountUpiDetails")}</h3>
                <div style={{ display:"flex",flexDirection:"column",gap:16 }}>
                  {[
                    { key:"accountHolderName",label:t("workerDashboard.accountHolderName"),placeholder:t("workerDashboard.asOnPassbook"),icon:"👤" },
                    { key:"bankName",          label:t("workerDashboard.bankName"),           placeholder:"e.g. State Bank of India",icon:"🏦" },
                    { key:"accountNumber",     label:t("workerDashboard.accountNumber"),      placeholder:"10-digit account number",icon:"🔢" },
                    { key:"ifscCode",          label:t("workerDashboard.ifscCodeLabel"), placeholder:"e.g. SBIN0001234",icon:"🏷️" },
                    { key:"upiId",             label:t("workerDashboard.upiIdLabel"),    placeholder:"e.g. name@upi",icon:"📱" },
                  ].map(f => (
                    <div key={f.key}>
                  ...
                  <button disabled={savingPayout} style={{ marginTop:8,padding:"13px",borderRadius:11,border:"none",background:savingPayout?"#a5b4fc":"linear-gradient(135deg,#2563eb,#3b82f6)",color:"#fff",fontSize:14,fontWeight:800,cursor:savingPayout?"not-allowed":"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:8,boxShadow:savingPayout?"none":"0 6px 20px rgba(37,99,235,.35)" }}
                    onClick={async () => {
                      if (!myProfile) return;
                      ...
                      try {
                        ...
                        setMyProfile(updated);
                        onToast(t("workerDashboard.payoutAccountSaved"));
                      } catch (e) { onToast(e.message || t("workerDashboard.failedSave"),"error"); }
                      finally { setSavingPayout(false); }
                    }}
                  >
                    {savingPayout ? t("common.saving") : `💾 ${t("workerDashboard.savePayoutAccount")}`}
                  </button>
                </div>
              </div>
            </>
