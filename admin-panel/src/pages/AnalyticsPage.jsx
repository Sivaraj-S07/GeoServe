  ...
  return (
    <div style={{ background: bg, border: `1.5px solid ${border}`, borderRadius: 18, padding: "20px 22px", position: "relative", overflow: "hidden", transition: "all .2s", cursor: "default" }}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div>
          <div style={{ fontSize: 26, fontWeight: 900, color: "#0c1a3d", letterSpacing: -1 }}>{value}</div>
...
function SectionHeader({ title, sub }) {
  return (
    <div style={{ marginBottom: 16 }}>
      <h2 style={{ fontSize: 16, fontWeight: 800, color: "#0c1a3d", margin: 0 }}>{title}</h2>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      {data.map((row, i) => (
        <div key={i} style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ width: 70, fontSize: 12, fontWeight: 700, color: "#0c1a3d", flexShrink: 0 }}>{fmtRs(row[valueKey])}</div>
        </div>
      ))}
    </div>
  );
...
function WithdrawModal({ balance, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!amt || amt <= 0) { setError(t("common.error")); return; }
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center" }}
      ...
      <div style={{ background: "#fff", borderRadius: 20, padding: "32px 36px", width: "100%", maxWidth: 420, boxShadow: "0 24px 60px rgba(0,0,0,.25)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
          <h2 style={{ fontSize: 18, fontWeight: 900, color: "#0c1a3d", margin: 0 }}>Record Withdrawal</h2>
        ...
        <div style={{ background: "#f0fdf4", border: "1.5px solid #bbf7d0", borderRadius: 10, padding: "10px 14px", marginBottom: 20 }}>
          ...
          <span style={{ fontSize: 16, fontWeight: 900, color: "var(--primary)" }}>{fmtRs(balance)}</span>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <button type="submit" disabled={loading} style={{ flex: 1, height: 42, borderRadius: 10, border: "none", background: loading ? "var(--green-border)" : "linear-gradient(135deg,var(--primary),#7c3aed)", color: "white", fontWeight: 800, fontFamily: "inherit", cursor: "pointer", fontSize: 14 }}>
...
export default function AnalyticsPage({ onToast }) {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (!silent) onToast(err.message || t("common.error"), "error");
    } finally {
    ...
    <div className="anim-fade" style={{ padding:"28px 32px" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 12, marginBottom: 28 }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          <button onClick={() => setShowWD(true)} style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 16px", borderRadius: 9, border: "none", cursor: "pointer", background: "linear-gradient(135deg,#2563eb,#3b82f6)", color: "white", fontWeight: 700, fontSize: 13, fontFamily: "inherit", boxShadow: "0 3px 10px rgba(37,99,235,.3)" }}>
            💸 Record Withdrawal
          </button>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(170px, 1fr))", gap: 16, marginBottom: 20 }}>
        <StatCard label="Wallet Balance"    value={fmtRs(wallet?.balance)}           sub="Available to withdraw"             grad="linear-gradient(135deg,#2563eb,#3b82f6)" bg="var(--green-bg)" border="var(--primary-border)" icon="💰" />
        <StatCard label="Total Earned"      value={fmtRs(wallet?.totalEarned)}       sub={`Platform commission ${commRate}`} grad="linear-gradient(135deg,var(--primary),#7c3aed)" bg="var(--primary-bg)" border="var(--primary-border)" icon="📈" />
        ...
        <StatCard label="This Month"        value={fmtRs(wallet?.monthEarnings)}     sub="Commission this month"             grad="linear-gradient(135deg,var(--blue),#3b82f6)" bg="var(--blue-bg)" border="#bfdbfe" icon="📅" />
        <StatCard label="Confirmed Bookings" value={fmt(confirmed)}                  sub="Fully completed & paid"            grad="linear-gradient(135deg,#2563eb,#3b82f6)" bg="var(--green-bg)" border="var(--primary-border)" icon="✅" />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(170px, 1fr))", gap: 16, marginBottom: 28 }}>
        ...
        <StatCard label="Online Workers" value={fmt(userStats?.onlineWorkers)} sub="Active in last 5 min"  grad="linear-gradient(135deg,#2563eb,#3b82f6)" bg="var(--primary-bg)" border="var(--green-border)" icon="🟡" />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 24 }}>
        <Card style={{ padding: 24 }}>
          ...
          {monthly.length === 0
            ...
            : <BarChart data={monthly} valueKey="amount" labelKey="month" colorGrad="linear-gradient(90deg,var(--primary),#7c3aed)" />
          }
        </Card>
          ...
          {topCats.length === 0
            ...
            : (
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {topCats.map((cat, i) => {
                  const colors = [["var(--primary-bg)","var(--primary-border)","var(--primary)"],["#f0fdf4","#bbf7d0","var(--primary)"],["#fff7ed","#fed7aa","#d97706"],["#fef2f2","#fecaca","#dc2626"],["#f5f3ff","#ddd6fe","#7c3aed"]];
                  ...
                  return (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 13, fontWeight: 700, color: "#0c1a3d", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{cat.category}</div>
                        <div style={{ fontSize: 11, color: "#94a3b8" }}>{fmt(cat.count)} bookings</div>
                      </div>
                      <div style={{ fontSize: 14, fontWeight: 800, color: "var(--primary)", flexShrink: 0 }}>{fmtRs(cat.total)}</div>
                    </div>
                  );
                })}
              </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 24 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {[
              ...
              <div key={row.label} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <div style={{ width: 72, fontSize: 12, fontWeight: 800, color: "#0c1a3d", textAlign: "right" }}>
            ...
            <div style={{ marginTop: 8, paddingTop: 12, borderTop: "1px solid #f1f5f9", display: "flex", justifyContent: "space-between" }}>
              ...
              <span style={{ fontSize: 14, fontWeight: 900, color: "#0c1a3d" }}>{fmt(userStats?.total)}</span>
            </div>
          </div>
        </Card>
        ...
        <Card style={{ padding: 24 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {[
              { label: "Current Balance",     value: fmtRs(wallet?.balance),          accent: "var(--primary)" },
              { label: "Total Ever Earned",   value: fmtRs(wallet?.totalEarned),      accent: "var(--primary)" },
              { label: "Today's Commission",  value: fmtRs(wallet?.todayEarnings),    accent: "#f59e0b" },
              { label: "This Month",          value: fmtRs(wallet?.monthEarnings),    accent: "var(--blue)" },
          ...
          <button onClick={() => setShowWD(true)} style={{ marginTop: 16, width: "100%", padding: "10px 0", borderRadius: 10, border: "none", background: "linear-gradient(135deg,#2563eb,#3b82f6)", color: "white", fontWeight: 700, fontSize: 14, fontFamily: "inherit", cursor: "pointer" }}>
            💸 Record Withdrawal
          </button>
        </Card>
      </div>
                ...
                {txns.slice(0, 20).map((t, i) => {
                  ...
                  return (
                    <tr key={t.id || i}>
                      ...
                      <td><span style={{ fontWeight: 800, color: isCredit ? "var(--primary)" : "#ef4444", fontSize: 14 }}>{isCredit ? "+" : ""}{fmtRs(t.amount)}</span></td>
                      <td>
