  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:9999, background:"rgba(15,23,42,.7)", backdropFilter:"blur(6px)", display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}
      ...
      <div style={{ background:"var(--surface)", borderRadius:22, width:"100%", maxWidth:560, boxShadow:"0 32px 100px rgba(0,0,0,.3)", overflow:"hidden", maxHeight:"90vh", display:"flex", flexDirection:"column" }}>
        ...
        {/* Header */}
        <div style={{ background:"linear-gradient(135deg,var(--primary-dark),var(--primary),var(--green))", padding:"18px 22px", flexShrink:0 }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            <div style={{ display:"flex", alignItems:"center", gap:12 }}>
              ...
              <div>
                <div style={{ fontSize:16, fontWeight:800, color:"white", fontFamily:"'Manrope',sans-serif" }}>{t("nav.navigateToCustomer")}</div>
          ...
          <div style={{ display:"flex", gap:4, marginTop:14, background:"rgba(0,0,0,.2)", borderRadius:10, padding:3 }}>
            {[["map","🗺️ Map & Navigate"],["contact","📞 Contact Info"]].map(([id,lbl])=>(
              <button key={id} onClick={()=>setTab(id)} style={{ flex:1, padding:"7px 0", borderRadius:8, background:tab===id?"white":"transparent", border:"none", cursor:"pointer", fontSize:12, fontWeight:700, color:tab===id?"var(--primary)":"rgba(255,255,255,.8)", fontFamily:"inherit" }}>{lbl}</button>
            ))}
          </div>
        </div>

        {/* Booking summary bar */}
        <div style={{ background:"#f0fdf4", borderBottom:"1px solid var(--primary-border)", padding:"10px 20px", display:"flex", gap:20, flexWrap:"wrap", flexShrink:0 }}>
          {[[t("nav.customerLabel"),booking.userName],["Service",booking.category||"General Service"],["Date",(() => { try { return new Date(booking.date).toLocaleDateString("en-IN",{day:"numeric",month:"short"}); } catch { return booking.date; } })()]].map(([lbl,val])=>(
            <div key={lbl} style={{ fontSize:12 }}>
              <span style={{ color:"#6b7280", fontWeight:600 }}>{lbl}: </span>
              <span style={{ color:"var(--primary-dark)", fontWeight:700 }}>{val}</span>
            </div>
          ))}
        </div>
              ...
              {hasUserLoc ? (
                <>
                  ...
                  <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                    <a href={mapsUrl} target="_blank" rel="noopener noreferrer" style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:10, background:"#4285F4", color:"white", borderRadius:12, padding:"13px 20px", textDecoration:"none", fontWeight:800, fontSize:14, fontFamily:"'Manrope',sans-serif", boxShadow:"0 4px 16px rgba(66,133,244,.35)" }}>
                    ...
                    {!workerLoc ? (
                      <button onClick={getMyLoc} disabled={gettingLoc} style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:8, background:"var(--surface)", color:"#2563eb", border:"1.5px solid #bfdbfe", borderRadius:12, padding:"11px 20px", fontWeight:700, fontSize:13, cursor:"pointer", fontFamily:"inherit" }}>
                        ...
                        {gettingLoc ? t("userDashboard.gettingLocation") : "Add My Location for Better Directions"}
                      </button>
                    ) : (
                      <div style={{ background:"var(--primary-bg)", border:"1px solid var(--primary-border)", borderRadius:10, padding:"9px 14px", fontSize:12, color:"var(--primary-dark)", display:"flex", alignItems:"center", gap:8 }}>
                        <Icon name="check-circle" size={13} color="var(--primary)" /> Your location added — Google Maps shows exact route
                      </div>
                    )}
                  </div>
                </>
                ...
                <div style={{ textAlign:"center", padding:"40px 24px", background:"#fffbeb", border:"1px solid #fde68a", borderRadius:14 }}>
                  ...
                  <div style={{ fontWeight:700, fontSize:15, color:"var(--text)", marginBottom:8 }}>{t("nav.customerLocationNotShared")}</div>
                  <p style={{ color:"#92400e", fontSize:13, lineHeight:1.6, marginBottom:16 }}>{t("nav.contactCustomerForAddress")}</p>
                  {booking.notes && (
                    <div style={{ background:"var(--surface)", border:"1px solid #fde68a", borderRadius:10, padding:"10px 14px", textAlign:"left" }}>
                      <div style={{ fontSize:10, fontWeight:700, color:"#92400e", marginBottom:4, textTransform:"uppercase" }}>{t("nav.customerNotes")}</div>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
              <div style={{ background:"var(--bg)", border:"1.5px solid var(--border)", borderRadius:14, padding:"16px 18px" }}>
                <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:14 }}>
                  <div style={{ width:44, height:44, borderRadius:12, background:"linear-gradient(135deg,#2563eb,#7c3aed)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:18, fontWeight:800, color:"white" }}>
                    {booking.userName?.charAt(0).toUpperCase()}
                  </div>
                  <div>
                    <div style={{ fontWeight:700, fontSize:14, color:"var(--text)" }}>{booking.userName}</div>
                    <div style={{ fontSize:11, color:"var(--muted)" }}>{t("nav.customerLabel")}</div>
                  </div>
                </div>
                {booking.userPhone ? (
                ...
                {booking.notes && (
                  <div style={{ background:"#fffbeb", border:"1px solid #fde68a", borderRadius:10, padding:"10px 14px", marginTop:8 }}>
                    <div style={{ fontSize:10, fontWeight:700, color:"#92400e", textTransform:"uppercase", marginBottom:4 }}>{t("nav.notesFromCustomer")}</div>
                    <div style={{ fontSize:13, color:"var(--text)" }}>{booking.notes}</div>
                  </div>
                )}
              </div>
              <div style={{ background:"#eff6ff", border:"1px solid #bfdbfe", borderRadius:10, padding:"10px 14px", fontSize:12, color:"#1d4ed8", display:"flex", alignItems:"flex-start", gap:8 }}>
                ...
                <span>{t("nav.contactInfoPrivacy")}</span>
              </div>
            </div>
          )}

          <button onClick={onClose} style={{ width:"100%", marginTop:14, padding:"11px", background:"var(--bg)", border:"1.5px solid var(--border)", borderRadius:10, fontWeight:700, fontSize:13, cursor:"pointer", fontFamily:"inherit", color:"var(--muted)" }}>{t("nav.close")}</button>
        </div>
      </div>
    </div>
  );
