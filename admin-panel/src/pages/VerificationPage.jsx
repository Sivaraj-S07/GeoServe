...
const STATUS_CFG = {
  ...
  verified: { label: "Verified", color: "var(--primary)", bg: "var(--green-bg)", border: "var(--primary-border)", icon: "✅" },
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          background: "linear-gradient(135deg, #f8fafc, var(--primary-bg))",
        }}>
          <div>
              ...
              {record.certificate_mimetype === "application/pdf" ? (
                ...
                <div style={{ textAlign: "center" }}>
                  ...
                  <div style={{ display:"none",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:40,color:"#94a3b8" }}>
                    ...
                    <a href={certUrl} target="_blank" rel="noreferrer" style={{ marginTop:14,color:"var(--primary)",fontWeight:700,fontSize:13 }}>Try opening directly ↗</a>
                  </div>
                </div>
              )}
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          background: isApprove ? "linear-gradient(135deg, var(--green-bg), #f0fdf4)" : "linear-gradient(135deg, #fef2f2, #fee2e2)",
          borderBottom: `1px solid ${isApprove ? "var(--primary-border)" : "#fecaca"}`,
        }}>
          <div style={{ fontSize: 36, marginBottom: 10 }}>{isApprove ? "✅" : "❌"}</div>
          <div style={{ fontSize: 18, fontWeight: 800, color: "var(--text)" }}>
            {isApprove ? t("adminVerification.approveWorker") : t("adminVerification.rejectWorker")}
          </div>
        ...
        <div style={{ padding: 24 }}>
          <label style={{ display: "block", fontSize: 13, fontWeight: 700, color: "#374151", marginBottom: 8 }}>
            {isApprove ? t("adminVerification.rejectReason") : t("adminVerification.rejectReason")}
          </label>
          <textarea
            value={notes}
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
            ...
            <button
              ...
              style={{
                ...
                boxShadow: isApprove ? "0 4px 16px rgba(37,99,235,.3)" : "0 4px 16px rgba(220,38,38,.3)",
              }}
            >
...
function DeleteConfirmModal({ record, busy, onConfirm, onClose }) {
  return (
    <div style={{
      position: "fixed", inset: 0, zIndex: 9100,
      background: "rgba(15,23,42,.7)", backdropFilter: "blur(4px)",
      ...
      <div style={{
        ...
        <div style={{ padding: 24 }}>
          <div style={{
            ...
            padding: "12px 14px", fontSize: 13, color: "#991b1b", lineHeight: 1.5,
          }}>
            This will <strong>permanently remove</strong> this verification request, its uploaded
            documents/videos, and reset the worker's verification status. This action cannot be undone.
          </div>
          <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
            <button onClick={onClose} disabled={busy} style={{
              flex: 1, padding: "12px", background: "var(--bg)",
              border: "1.5px solid var(--border)", borderRadius: 10, fontWeight: 700,
              fontSize: 14, cursor: busy ? "not-allowed" : "pointer", color: "var(--muted)", fontFamily: "inherit",
              opacity: busy ? 0.7 : 1,
            }}>Cancel</button>
            <button
              onClick={onConfirm}
              ...
              style={{
                ...
                color: "white", border: "none", borderRadius: 10,
                fontWeight: 800, fontSize: 14, cursor: busy ? "not-allowed" : "pointer", fontFamily: "inherit",
                opacity: busy ? 0.7 : 1,
...
function VerifCard({ record, categories, onApprove, onReject, onDelete, onPreview, busy }) {
      ...
      <div style={{ padding: "20px 22px" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button
            ...
            disabled={busy}
            title="Permanently delete this verification request"
            style={{
              flex: record.verification_status === "pending" ? "0 0 auto" : 1,
              width: record.verification_status === "pending" ? 42 : undefined,
              padding: "10px 8px",
              background: "var(--surface)",
              border: "1.5px solid var(--border)", borderRadius: 10, fontWeight: 700, fontSize: 13,
              cursor: busy ? "not-allowed" : "pointer", color: "#dc2626", fontFamily: "inherit",
              display: "flex", alignItems: "center", justifyContent: "center", gap: 6,
              opacity: busy ? 0.7 : 1,
            }}
          >🗑️{record.verification_status !== "pending" ? " Delete" : ""}</button>
        </div>
      </div>
    </div>
  );
...
export default function VerificationPage({ onToast }) {
  ...
  const [deleteTarget, setDeleteTarget] = useState(null); // record
  ...
  const handleDelete = async () => {
    ...
    setBusy(true);
    try {
      await api.deleteVerification(deleteTarget.workerId);
      onToast(`🗑️ Verification request for ${deleteTarget.workerName} was deleted.`);
      setDeleteTarget(null);
      load(true);
    } catch (e) {
      onToast(e.message || "Failed to delete verification request", "error");
    } finally { setBusy(false); }
  };
    ...
    <div className="anim-fade" style={{ padding:"28px 32px" }}>
      ...
      {stats && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 14, marginBottom: 28 }}>
          {[
            { label: "Total Requests", value: stats.total,    bg: "var(--primary-bg)", border: "#c7d2fe", color: "var(--primary)", icon: "📋" },
            { label: "Pending Review", value: stats.pending,  bg: "#fffbeb", border: "#fde68a", color: "#d97706", icon: "⏳" },
            { label: "Verified",       value: stats.verified, bg: "var(--green-bg)", border: "var(--primary-border)", color: "var(--primary)", icon: "✅" },
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 24, flexWrap: "wrap" }}>
        {["all", "pending", "verified", "rejected"].map(f => {
          const cfg = f === "all" ? { label: "All", color: "var(--primary)", bg: "var(--primary-bg)", border: "#c7d2fe" } : STATUS_CFG[f];
          ...
          return (
            <button key={f} onClick={() => setFilter(f)} style={{
              ...
              background: isActive ? (f === "all" ? "linear-gradient(135deg, var(--primary), #7c3aed)" : f === "pending" ? "linear-gradient(135deg, #d97706, #f59e0b)" : f === "verified" ? "linear-gradient(135deg, var(--primary), #34d399)" : "linear-gradient(135deg, #dc2626, #ef4444)") : "white",
