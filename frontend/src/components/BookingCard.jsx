          ...
          {payMode === "upi" && (
            <>
              ...
              <div style={{ background:"var(--bg)", borderRadius:14, padding:"20px", border:"1.5px solid #bfdbfe", textAlign:"center", marginBottom:16 }}>
                {upiId ? (
                  <>
                    <QRCodeDisplay upiId={upiId} amount={amount} workerName={workerProfile?.name || "Worker"} />
    ...
    <div style={{ background:"linear-gradient(135deg,#f0fdf4,var(--primary-bg))", border:"2px solid #86efac", borderRadius:16, padding:"20px 22px", marginBottom:16 }}>
      ...
      {payMode === "qr" && (
        <div>
          ...
          <button disabled={busy} onClick={() => onUserConfirm("upi")}
...
export default function BookingCard({
  ...
  const handlePaymentConfirm = async (paymentMode) => {
    await act(async () => {
      ...
      if (onConfirm) await onConfirm(booking.id, paymentMode);
    });
    setShowPaymentModal(false);
  };

  // Payment confirm for bookings already in "completed" state (payment screen inside card)
  const handleInCardConfirm = (paymentMode) => act(() => onConfirm && onConfirm(booking.id, paymentMode));
        ...
        {showPaymentScreen && (
          <div style={{ padding:"16px 20px 0" }}>
            <PaymentScreen
              ...
              workerProfile={workerProfile}
              busy={busy}
              onUserConfirm={(mode) => handleInCardConfirm(mode)}
            />
          </div>
        )}
