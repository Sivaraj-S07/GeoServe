...
import Icon, { CategoryChip, CategoryLabel } from "./Icon";
import PincodeSelector from "./PincodeSelector";
import { getLocalizedName } from "../utils/localizedName";

const BLANK = {
  name: "", nameEn: "", categoryId: "", specialization: "", bio: "",
...
export default function WorkerModal({ worker, categories, onSave, onClose }) {
  ...
  useEffect(() => {
    if (worker) {
      setF({
        name:           worker.name || "",
        nameEn:         worker.nameEn || worker.name || "",
  ...
  const validate = () => {
    const e = {};
    if (!f.nameEn.trim())         e.nameEn = t("common.submit") === "Submit" ? "Required" : "தேவை";
    if (!f.categoryId)            e.categoryId = t("common.submit") === "Submit" ? "Required" : "தேவை";
    if (!f.specialization.trim()) e.specialization = t("common.submit") === "Submit" ? "Required" : "தேவை";
    if (!f.phone.trim())          e.phone = t("common.submit") === "Submit" ? "Required" : "தேவை";
  ...
  const handleSave = async () => {
    ...
    try {
      await onSave({
        ...
        categoryId: parseInt(f.categoryId), lat: parseFloat(f.lat)||13.0827, lng: parseFloat(f.lng)||80.2707, yearsOfExp: parseInt(f.yearsOfExp)||0,
      });
    } finally { setBusy(false); }
  };

  const previewName = f.nameEn || "W";
  const fb = `https://ui-avatars.com/api/?name=${encodeURIComponent(previewName)}&background=16a34a&color=fff&size=80`;
      ...
      <div style={{ background:"var(--surface)", borderRadius:20, width:"100%", maxWidth:600, boxShadow:"0 24px 80px rgba(0,0,0,.25)", overflow:"hidden", maxHeight:"90vh", display:"flex", flexDirection:"column" }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ background:"linear-gradient(135deg,#1e3a8a,#2563eb)", padding:"20px 24px", display:"flex", alignItems:"center", justifyContent:"space-between", flexShrink:0 }}>
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            ...
            <div>
              <div style={{ fontSize:16, fontWeight:800, color:"white", fontFamily:"'Manrope',sans-serif" }}>{isEdit ? "Edit Profile" : "Create Worker Profile"}</div>
              <div style={{ fontSize:12, color:"rgba(255,255,255,.75)", marginTop:1 }}>{isEdit ? `Editing ${getLocalizedName(worker, i18n.language) || worker.name}` : "Fill in your professional details"}</div>
            </div>
          </div>
        ...
        <div style={{ overflowY:"auto", flex:1, padding:"24px" }}>
          ...
          <div style={{ marginBottom:20 }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:16, marginBottom:12 }}>
              ...
              <div>
                <div style={{ fontWeight:700, fontSize:14, color:"var(--text)", marginBottom:2 }}>{f.nameEn||"Worker Name"}</div>
                <div style={{ fontSize:12, color:"var(--muted)", display:"flex", alignItems:"center", gap:4 }}>
                  {(() => { const cat = categories.find(c=>c.id===parseInt(f.categoryId)); return cat ? <CategoryLabel name={cat.name} icon={cat.icon} size={11} color="var(--muted)" /> : "Category"; })()}
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:14, marginBottom:20 }}>
            <FL label="Name" req err={errs.nameEn}>
              <input style={inp(errs.nameEn)} value={f.nameEn} onChange={e=>set("nameEn",e.target.value)} placeholder="Your name in English" />
            </FL>
              ...
              <FL label="Category" req err={errs.categoryId}>
                <div style={{ position: "relative" }}>
                  <span style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", pointerEvents: "none", zIndex: 1, display: "flex" }}>
                    {f.categoryId
                      ? <CategoryLabel name="" icon={categories.find(c => String(c.id) === String(f.categoryId))?.icon} size={14} color="var(--primary)" showName={false} />
                      : <Icon name="layers" size={14} color="var(--muted)" />}
                  </span>
                  <select style={{ ...inp(errs.categoryId), paddingLeft: 30 }} value={f.categoryId} onChange={e => set("categoryId", e.target.value)}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
              ...
              {worker.aadhaarNumber && (
                <FL label="Aadhaar Number">
                  <input
                    type="text"
                    style={{ ...inp(false), letterSpacing: 2, background: 'var(--surface-hover)' }}
                    value={worker.aadhaarNumber.replace(/(\d{4})(\d{4})(\d{4})/, '$1 $2 $3')}
                    readOnly
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:14, marginBottom:20 }}>
            ...
            <FL label="Experience Description">
              ...
              />
              <span style={{ fontSize:11, color:"var(--muted)", marginTop:4, display:"block" }}>{t("workerModal.experienceTip")}</span>
            </FL>
          ...
          <div style={{ background:"var(--bg)", border:"1.5px solid var(--border)", borderRadius:10, padding:"12px 16px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            <div>
              <div style={{ fontSize:13, fontWeight:700, color:"var(--text)" }}>{t("workerModal.availableForBookings")}</div>
              <div style={{ fontSize:12, color:"var(--muted)" }}>{t("workerModal.customersCanBook")}</div>
            </div>
        ...
        <div style={{ padding:"16px 24px", borderTop:"1px solid #e2e8f0", display:"flex", gap:10, justifyContent:"flex-end", background:"var(--bg)", flexShrink:0 }}>
          <button onClick={onClose} style={{ padding:"10px 20px", borderRadius:9, border:"1.5px solid var(--border)", background:"var(--surface)", fontSize:13, fontWeight:700, cursor:"pointer", color:"var(--muted)", fontFamily:"inherit" }}>{t("workerModal.cancel")}</button>
          <button onClick={handleSave} disabled={busy} style={{ padding:"10px 24px", borderRadius:9, border:"none", background:busy?"#a5b4fc":"linear-gradient(135deg,#2563eb,#2563eb)", color:"white", fontSize:13, fontWeight:800, cursor:busy?"not-allowed":"pointer", fontFamily:"'Manrope',sans-serif", display:"flex", alignItems:"center", gap:8, boxShadow:busy?"none":"0 4px 14px rgba(37,99,235,.35)" }}>
