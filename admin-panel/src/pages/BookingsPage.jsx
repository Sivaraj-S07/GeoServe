...
const STATUS_META = {
  ...
  accepted:    { color: "var(--blue)", bg: "var(--blue-bg)", border: "#bfdbfe", label: "Accepted",    dot: "#3b82f6" },
  in_progress: { color: "#7c3aed", bg: "#f5f3ff", border: "#ddd6fe", label: "In Progress", dot: "#a78bfa" },
  completed:   { color: "var(--primary)", bg: "var(--green-bg)", border: "var(--primary-border)", label: "Completed",   dot: "var(--green)" },
    ...
    <div style={{ position: "relative" }}>
      <button
        ...
        style={{
          ...
          background: rows.length === 0 ? "#bbf7d0" : "var(--green)",
          ...
          <div style={{
            ...
            {[
              ...
              >
                ...
                <div>
                  <div style={{ fontSize: 13, fontWeight: 700, color: "#0c1a3d" }}>{item.label}</div>
  ...
  return (
    ...
    >
      ...
      <div style={{ paddingTop: 4 }}>
        <div style={{ fontSize: 22, fontWeight: 900, letterSpacing: -1, color: "#0c1a3d" }}>{value}</div>
...
export default function BookingsPage({ onToast }) {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (!silent) onToast(t("common.error"), "error");
    } finally {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      onToast(t("adminBookings.noBookings"));
    } catch {
  ...
  const filtered = bookings.filter(b => {
    ...
    const mQ = !q ||
      ...
      b.userNameEn?.toLowerCase().includes(q) ||
      b.workerName?.toLowerCase().includes(q) ||
      b.workerNameEn?.toLowerCase().includes(q) ||
  ...
  return (
    <div className="anim-fade" style={{ padding: "32px 28px" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between",
        ...
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 900, letterSpacing: "-.5px", margin: 0, color: "#0c1a3d" }}>
            📅 Booking Management
          </h1>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(140px,1fr))", gap: 14, marginBottom: 24 }}>
        <StatCard label="Total"       value={stats.total}                            grad="linear-gradient(135deg,var(--primary),#7c3aed)"  bg="var(--primary-bg)"  border="#c7d2fe" />
        ...
        <StatCard label="Completed"   value={stats.done}                             grad="linear-gradient(135deg,#2563eb,#3b82f6)"  bg="var(--green-bg)"  border="var(--primary-border)" />
        <StatCard label="Revenue"     value={`₹${stats.revenue.toLocaleString()}`}   grad="linear-gradient(135deg,var(--blue),#3b82f6)"  bg="var(--blue-bg)"  border="#bfdbfe" />
      ...
      <div style={card}>
        {loading ? (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: 64, gap: 14 }}>
            <div style={{ width: 32, height: 32, borderRadius: "50%", border: "3px solid var(--primary-bg)",
              borderTopColor: "var(--primary)", animation: "spin .7s linear infinite" }} />
            <span style={{ color: "#64748b", fontWeight: 600 }}>Loading bookings…</span>
          </div>
        ) : filtered.length === 0 ? (
          <div style={{ textAlign: "center", padding: "64px 24px" }}>
            <div style={{ fontSize: 48, marginBottom: 12, opacity: .6 }}>📋</div>
            <div style={{ fontWeight: 700, fontSize: 16, color: "#0c1a3d", marginBottom: 6 }}>No bookings found</div>
                  ...
                  >
                    <td>
                      <div style={{ fontWeight: 700, fontSize: 13, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        {getLocalizedName({ name: b.userName, nameEn: b.userNameEn }, i18n.language) || "—"}
                      </div>
                    ...
                    <td>
                      <div style={{ color: "#475569", fontSize: 13, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        {getLocalizedName({ name: b.workerName, nameEn: b.workerNameEn }, i18n.language) || "—"}
                      </div>
                    </td>
                    <td>
                      {b.category && (
                        <span style={{ background: "var(--blue-bg)", border: "1px solid #bfdbfe", color: "#1d4ed8",
                    ...
                    <td><div style={{ fontWeight: 800, color: "var(--primary)", fontSize: 14 }}>₹{(b.cost || 0).toLocaleString()}</div></td>
                    <td>
                      <span style={{
                        background: b.paymentStatus === "paid" ? "var(--green-bg)" : "#f8fafc",
                        border: `1px solid ${b.paymentStatus === "paid" ? "var(--primary-border)" : "#e2e8f0"}`,
                        color: b.paymentStatus === "paid" ? "var(--primary)" : "#94a3b8",
                        fontSize: 11, fontWeight: 700, padding: "4px 10px", borderRadius: 20,
                      ...
                      <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                        <button
                          ...
                          style={{
                            ...
                            background: expanded === b.id ? "var(--primary-bg)" : "white",
                            color: expanded === b.id ? "var(--primary)" : "#64748b",
                      ...
                      <td colSpan={9} style={{
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(200px,1fr))", gap: "14px 24px" }}>
                          {[
                            ...
                            ["Pay Method",    b.paymentMode || "—"],
                            ["Paid At",       b.paidAt ? fmtFull(b.paidAt) : "—"],
                            ["Customer UPI Ref", b.customerPaymentRef || "—"],
                            ...
                            <div key={label}>
                              ...
                              <div style={{ fontSize: 13, color: "#0c1a3d", fontWeight: 500, wordBreak: "break-all" }}>
                                {String(value)}
                              </div>
                            </div>
                          ))}
